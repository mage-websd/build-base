...
class Fishpig_Wordpress_TermController extends Fishpig_Wordpress_Controller_Abstract
{
	...
	public function viewAction()
	{
		...
		if($this->getRequest()->isXmlHttpRequest()) {
			$response = array();
			$termBlock = $this->getLayout()->getBlock('wp.term');
			$postListBlock = $termBlock->getPostListBlock()->setTemplate('wordpress/ajax/post/list.phtml');
			$pagerBlock = $postListBlock->getPagerBlock();
			$response['data']['post_list'] = $postListBlock->toHtml();
			$response['data']['pager'] = $pagerBlock->toHtml();
			$response['success'] = 1;
