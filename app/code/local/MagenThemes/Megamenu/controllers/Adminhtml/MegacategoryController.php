...
?>
<?php
class MagenThemes_Megamenu_Adminhtml_MegacategoryController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction() {
		...
		$megamenu  = Mage::getModel('megamenu/megacategory')->load($id);
		...
		if ($this->getRequest()->getQuery('isAjax')) {
		    ...
		    if(!$id)
			$result['tree'] = $this->getLayout()->createBlock('megamenu/adminhtml_megacategory_tree')->getTreeHtml();
    ...
    private function _importCategory($category)
    {
		...
		$parentMegamenu = Mage::getModel('megamenu/megacategory')->loadByCategoryId($parentCategoryId);
		$megamenu = Mage::getModel('megamenu/megacategory')
    ...
    {
		$root = Mage::getModel('megamenu/megacategory')
				->getCollection()
				->addFieldToFilter('parent_id', 0)
				->setStoreFilter($this->getRequest()->getParam('store_id'));
		if(!count($root)) {
		    $existsIds = array();
		    $existsCategory = Mage::getModel('megamenu/megacategory')
					->getCollection()
    ...
    public function moveAction()
    {
		...
		if($ids) {
		    ...
		    try{
				$move = Mage::getModel('megamenu/megacategory')->load($moveId);
				if($parentId != 0 && $move->getParentId() != 0) {
				    ...
				    foreach($ids as $id) {
				    	if($id) {
				    		if(Mage::getModel('megamenu/megacategory')->load($id)->getParentId() != 0){
				    			Mage::getModel('megamenu/megacategory')->load($id)->setPosition($i)->save();
				    			$i++;
				    		}
				    	}
				    }
    ...
    public function saveAction()
    {
		...
		if ($data = $this->getRequest()->getPost()) {
		    $model = Mage::getModel('megamenu/megacategory');
		    ...
		    if($data['megamenu']['parent_id']) {
				$level = Mage::getModel('megamenu/megacategory')->load($data['megamenu']['parent_id'])->getLevel();
				$level += 1;
