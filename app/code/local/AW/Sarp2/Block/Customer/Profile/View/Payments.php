...
class AW_Sarp2_Block_Customer_Profile_View_Payments extends Mage_Core_Block_Template
{
    ...
    public function getInfoBoxFields()
    {
        ...
        if ($item->getTypeModel()->getTrialIsEnabled()) {
            if ($profile->getData('details/order_item_info/subscription_trial_payment')) {
                $fields[] = array(
                    'title' => $this->__('Trial Amount:'),
                    'value' => $currency->format($profile->getData('details/order_item_info/subscription_trial_payment'), array(), false),
                );
            } else {
                $fields[] = array(
                    'title' => $this->__('Trial Amount:'),
            ...
            $fields[] = array(
                'title' => $this->__('Trial Shipping Amount:'),
                'value' => $currency->format($profile->getData('details/shipping_amount'), array(), false),
            );
            if ($profile->getData('details/order_item_info/base_subscription_trial_tax')) {
                $fields[] = array(
                    'title' => $this->__('Trial Tax Amount:'),
                    'value' => $currency->format($profile->getData('details/order_item_info/base_subscription_trial_tax'), array(), false),
                );
            }
        }
