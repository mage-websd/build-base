    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    protected function _initForm()
    {
        ...
        if ($engineModel->getPaymentRestrictionsModel()->isInfiniteSupported()) {
            $generalFieldset->addField(
                'period_is_infinite',
                'select',
                array(
            ...
            $generalFieldset->addField(
                'infinite_is_enabled_label',
                'label',
                array(
                    'required' => false,
                    'name'     => 'infinite_is_enabled_label',
                    'label'    => 'Is Infinite',
                )
            );
        }
    ...
    protected function _setFormValues()
    {
        ...
        if (Mage::registry('current_type')) {
            ...
            $data['trial_is_enabled_label'] = $this->__('Engine does not support Trial Period');
            $data['initial_fee_is_enabled_label'] = $this->__('Engine does not support Initial Fee');
            $data['infinite_is_enabled_label'] = $this->__('Engine does not support Infinite Subscriptions');
            $form->setValues($data);
        }
        return $this;
    }
        ...
        return
            '<script type="text/javascript">
                Event.observe(document, "dom:loaded", function(e) {' .
                    (Mage::registry('current_type')->getEngineModel()->getPaymentRestrictionsModel()->isInfiniteSupported() ?
