...
class AW_Sarp2_Block_Adminhtml_Profile_View_Tab_Info_Payments extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _initForm()
    {
        ...
        if ($item->getTypeModel()->getTrialIsEnabled()) {
            if ($this->getProfile()->getData('details/order_item_info/subscription_trial_payment')) {
                $fieldset->addField(
                    'trial_amount',
                    'label',
                    array(
                        ...
                        'value' => $this->_formatPrice(
                            $this->getProfile()->getData('details/order_item_info/subscription_trial_payment')
                        ),
            ...
            $fieldset->addField(
                ...
                'label',
                array(
                    'name'  => 'trial_shipping_amount',
                    'value' => $this->_formatPrice($this->getProfile()->getData('details/shipping_amount')),
                    'label' => $this->__('Trial Shipping Amount'),
                    'bold'  => true,
                )
            );
            if ($this->getProfile()->getData('details/order_item_info/base_subscription_trial_tax')) {
                $fieldset->addField(
                    'trial_tax_amount',
                    'label',
                    array(
                        'name' => 'trial_tax_amount',
                        'value' => $this->_formatPrice(
                            $this->getProfile()->getData('details/order_item_info/base_subscription_trial_tax')
                        ),
                        'label' => $this->__('Trial Tax Amount'),
                        'bold' => true,
                    )
                );
            }
