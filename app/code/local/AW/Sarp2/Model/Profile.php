...
class AW_Sarp2_Model_Profile extends Mage_Core_Model_Abstract
{
    ...
    protected $_lastNotificationStatus = null;
    ...
    public function createOrder()
    {
        ...
        foreach ($itemInfoObjects as $itemInfo) {
            ...
            $billingAmount += $item->getBaseRowTotal();
    ...
    private function _getRegularItem(Varien_Object $itemInfo)
    {
        ...
        $taxAmount = $price * $this->getData('details/order_item_info/tax_percent') / 100;
        ...
        $item = Mage::getModel('sales/order_item')
            ...
            ->setBaseOriginalPrice($this->getData('details/order_item_info/base_original_price'))
            ->setPrice($store->convertPrice($this->getData('details/order_item_info/price')))
            ->setBasePrice($this->getData('details/order_item_info/base_price'))
    ...
    /**
     * Checks if notification about profile status already sent
     *
     * @param $status
     * @return bool
     */
    public function isAlreadyNotified($status)
    {
        if ($this->_lastNotificationStatus == $status) {
            return true;
        }
        return false;
    }
     ...
     * @return $this
     */
    public function setLastNotificationStatus($status)
    {
        $this->_lastNotificationStatus = $status;
        return $this;
    }
}