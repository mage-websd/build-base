...
class AW_Sarp2_Model_Resource_Profile_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function addStatusesToFilter(array $statuses)
    {
        $globalStatuses = Mage::getModel('aw_sarp2/source_profile_status')->getGlobalStatuses();
        $conditions = array();
        ...
        if (is_array($statuses)) {
            foreach ($statuses as $status) {
                if (array_key_exists($status, $globalStatuses)) {
                    ...
                    list($engineCode, $statusValue) = explode(AW_Sarp2_Model_Source_Profile_Status::STATUS_DELIMETER, $status);
                    $conditions[] = $this->_quoteInto($statusValue, $engineCode);
                }
            }
        }
        $this->getSelect()->where(implode(' OR ', $conditions));
