    ...
    extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        $priceIncludesTax = Mage::getSingleton('tax/config')->priceIncludesTax($store);
            ...
            if (Mage::helper('aw_sarp2/quote')->isQuoteItemIsSubscriptionProduct($item)) {
                ...
                if (
                    ...
                    if (Mage::helper('aw_sarp2/config')->isTaxOnTrialEnabled($store)) {
                        if ($priceIncludesTax) {
                            $trialPrice = $item->getRowTotal() * $subscriptionItem->getData('trial_price') / $subscriptionItem->getData('regular_price');
                        } else {
                            $trialPrice = $subscriptionItem->getData('trial_price') * $item->getQty();
                        }
                    } else {
                        $trialPrice = $subscriptionItem->getData('trial_price') * $item->getQty();
                    }
                    $item->setData(
                            ...
                            (float)$address->getQuote()->getStore()->convertPrice(round($trialPrice, 2))
                        );
