        ...
        foreach ($items as $item) {
            if (!$item->getProduct()->isVirtual() && !$item->getFreeShipping()) {
                ...
                if ($address->getShippingAmount()) {
                    if ($address->getShippingAmount() != 0 && $address->getBaseShippingAmount() == 0) {
                        $shipping = $address->getShippingAmount();
                        $address->setBaseShippingAmount($shipping);
                        $address->setShippingAmount($address->getQuote()->getStore()->convertPrice($shipping, false));
                    }
                    $item->setBaseShippingAmount($address->getBaseShippingAmount());
                    $item->setShippingAmount($address->getShippingAmount());
                }
            }
        }
        return $this;
