...
class AW_Sarp2_Model_Sales_Quote_Address extends Mage_Sales_Model_Quote_Address
{
    ...
    public function getBaseSubtotalWithDiscount()
    {
        if (Mage::helper('aw_sarp2/quote')->isQuoteHasSubscriptionProduct($this->getQuote())) {
            ...
            $result = 0;
            foreach ($items as $item) {
                $result += $item->getBaseRowTotal() - $item->getBaseDiscountAmount();
            }
            return $result;
        }
    ...
    public function getItemQty($itemId = 0)
    {
        ...
        return parent::getItemQty();
    }

    public function format($type)
    {
        if (Mage::helper('aw_sarp2')->checkMageVersion('1.5.0.0', '<')) {
            $data = $this->getData();
