...
class AW_Sarp2_Model_Sales_Quote_Address_Total_Subscription_Initial
    extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    protected $_itemRowTotalKey = 'subscription_initial_fee_payment';
        ...
        foreach ($items as $item) {
            if (Mage::helper('aw_sarp2/quote')->isQuoteItemIsSubscriptionProduct($item)) {
                ...
                if (
                    ...
                    && $subscriptionItem->getTypeModel()->getInitialFeeIsEnabled()
                    && $subscriptionItem->getTypeModel()->getEngineModel()
                    && $subscriptionItem->getTypeModel()->getEngineModel()->getPaymentRestrictionsModel()
                        ->isInitialAmountSupported()
                ) {
                    $item->setData(
                        $this->_itemRowTotalKey,
                        $address->getQuote()->getStore()->convertPrice($subscriptionItem->getData('initial_fee_price'))
                    );
                }
            }
        }
    ...
    {
        return Mage::helper('aw_sarp2')->__('Initial Fee');
    }
}