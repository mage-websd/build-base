...
class AW_Sarp2_Model_Engine_Paypal_Service
{
    ...
    protected function _runRequest($command)
    {
        ...
        $this->_initRegionRequiredFieldByCountry($command, $requestData);
     ...
     * @return $this
     */
    private function _initRegionRequiredFieldByCountry($command, $requestData)
    {
        if (!array_key_exists('COUNTRYCODE', $requestData) || !empty($requestData['STATE'])) {
            return $this;
        }
        $countryModel = Mage::getModel('directory/country')->loadByCode($requestData['COUNTRYCODE']);
        if ($countryModel->getRegionCollection()->getSize() > 0) {
            return $this;
        }
        $this->_serviceRequestRequiredFieldsMap[$command] = array_diff(
            $this->_serviceRequestRequiredFieldsMap[$command], array('STATE')
        );
        $this->_serviceRequestEachCallRequiredFields = array_diff(
            $this->_serviceRequestEachCallRequiredFields, array('STATE')
        );
        return $this;
    }
