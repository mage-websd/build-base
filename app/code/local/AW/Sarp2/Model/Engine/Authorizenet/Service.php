...
class AW_Sarp2_Model_Engine_Authorizenet_Service
{
    ...
    const API_URL_TEST_MODE = 'https://apitest.authorize.net/xml/v1/request.api';
    ...
    protected function _toXml($fieldName, $fieldValue)
    {
        ...
        if (is_array($fieldValue)) {
            ...
            $xml .= $this->_filterXmlFieldValue($fieldValue);
        }
        if ($fieldName) {
            $xml .= "</{$fieldName}>";
        }
    ...
    protected function _filterXmlFieldValue($value = null)
    {
        $value = (string)$value;
        $value = str_replace(
            array('&', '"', "'", '<', '>'),
