    ...
    extends Mage_Bundle_Block_Catalog_Product_View_Type_Bundle_Option_Checkbox
{
    ...
    protected function _construct()
    {
        ...
        if($product) {
            $attributeSet = $product->getData('attribute_set_id');
            $attributeSet = Mage::getModel('eav/entity_attribute_set')->load($attributeSet);
            $attributeSet = $attributeSet->getAttributeSetName();
            $attributeSet = strtolower($attributeSet);
            if($attributeSet == 'cook') {
                $this->setTemplate('bundleextended/catalog/product/view/type/bundle/option/checkbox_cook.phtml');
                return $this;
            }
        }
