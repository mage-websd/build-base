...
class Emosys_Seller_Model_Resource_Review extends Mage_Core_Model_Resource_Db_Abstract
{
    public function _construct()
    {
        $this->_init('seller/review', 'review_id');
    }
    ...
    public function getRatingArray($object)
    {
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('seller/ratingentity'))
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object)
    {
        ...
        $adapter->delete($this->getTable('seller/ratingentity'), 'review_id=' . $object->getId());
    ...
    private function __saveToRating($object)
    {
        ...
        if (count($object->getData()) > 0) {
            $ratingTable = $this->getTable('seller/ratingentity');
            ...
            try {
                ...
                if(count($data) > 0) {
                    $ratingCollection = Mage::getModel('seller/ratingentity')->getCollection()
